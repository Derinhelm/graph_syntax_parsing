...
class TrainScores:
    ...
    def _calculate_left(self, config, irels):
        ...
        left_scores = \
            [(rel,
              LEFT_ARC,
              self.scrs[2 + j * 2] + self.uscrs[2],
              self.non_detach_scrs[2 + j * 2] + self.non_detach_uscrs[2])
                for j, rel in enumerate(irels)]

        if left_cost == 0:
            left_valid_scores = \
                [(rel, trans, sc, non_detach_sc)
                 for (rel, trans, sc, non_detach_sc) in left_scores
    ...
    def _calculate_right(self, config, irels):
        ...
        if right_cost == 0:
            right_valid_scores = \
                [(rel, trans, sc, non_detach_sc)
                 for (rel, trans, sc, non_detach_sc) in right_scores
                 if rel == config.get_stack_last_element().relation]
            right_wrong_scores = \
                [(rel, trans, sc, non_detach_sc)
                 for (rel, trans, sc, non_detach_sc) in right_scores
    ...
    def _calculate_shift(self, config):
        ...
        shift_scores = [(None, SHIFT, self.scrs[0] + self.uscrs[0],
                         self.non_detach_scrs[0] + self.non_detach_uscrs[0])]
    ...
    def _calculate_swap(self, config):
        ...
        swap_scores = [(None,
                        SWAP,
                        self.scrs[1] + self.uscrs[1],
                        self.non_detach_scrs[1] + self.non_detach_uscrs[1])]
    ...
    def create_valid_wrong(self, config, irels):
        left_valid, left_wrong = self._calculate_left(config, irels)
        right_valid, right_wrong = self._calculate_right(config, irels)
        shift_valid, shift_wrong, shift_case = self._calculate_shift(config)
        swap_valid, swap_wrong, swap_cost = self._calculate_swap(config)

        valid = chain(left_valid, right_valid,
                      shift_valid, swap_valid)
        wrong = chain(
            left_wrong, right_wrong,shift_wrong, swap_wrong,
            [(None, 4, -float('inf'), -float('inf'))])
        # TODO: -float('inf') isn`t in computational graph
        # (None, 4, -float('inf')) is used to ensure that at least one element will be.

        return valid, wrong, shift_case, swap_cost

    def choose_best(self, best_valid, best_wrong, swap_cost, dynamic_oracle):
        #force swap
        if swap_cost == 0:
            best = best_valid
        else:
        ...
        #1: might want to experiment with that parameter
            if best_wrong[1] == SWAP:
                best = best_valid
            else:
                if not dynamic_oracle:
                    best = best_valid
                elif best_valid[2] - best_wrong[2] > 1.0:
                    best = best_valid
                elif (best_valid[2] > best_wrong[2] and
                      random.random() > 0.1):
                    best = best_valid
                else:
                    best = best_wrong
...
class TestScores:
    ...
    def test_evaluate(self, config, irels):
        ...
        s2 = s2 + self.uscrs[3]
        ret = \
            [ [(r1, LEFT_ARC, s1) ] if left_arc_conditions else [],
              [(r2, RIGHT_ARC, s2) ] if right_arc_conditions else [],
              [(None, SHIFT, self.scrs[0] + self.uscrs[0])]
                if shift_conditions else [],
              [(None, SWAP, self.scrs[1] + self.uscrs[1])]
                if swap_conditions else []
            ]
        return ret
