...
class GNNNet:
    ...
    def evaluate(self, graph):
        ...
        ts = time.time()
        uscrs_cpu = uscrs['node'].clone().detach().cpu()
        time_logger.info(f"Time of detach_cpu for unlabeled: {time.time() - ts}")
        ts = time.time()
        uscrs = torch.sum(uscrs_cpu, dim=0)
        ...
        ts = time.time()
        scrs_cpu = scrs['node'].clone().detach().cpu()
        time_logger.info(f"Time of detach_cpu for labeled: {time.time() - ts}")
        ts = time.time()
        scrs = torch.sum(scrs_cpu, dim=0)
